...
class APIServer:
    ...
    def stream_add(self, body: StreamAdd):
        ...
        if "rtsp://" != stream_url[:7]:
            raise HTTPException(status_code=400, detail=f"Livestream URL must be an RTSP link starting with 'rtsp://'. {stream_url}")
        else:
            stream_id = str(uuid4()).strip()
